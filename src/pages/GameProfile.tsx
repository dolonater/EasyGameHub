...
interface GameAchievementsSummary {
  percentage: string;
  total: number;
  unlocked: number;
...
const gameProfileAchievementsSummaryCache = createRouteSessionCache<GameAchievementsSummary | null>();
...
export default function GameProfile() {
  ...
  const [achievementsSummary, setAchievementsSummary] = useState<GameAchievementsSummary | null | undefined>(undefined);
  const [achievementsSummaryLoading, setAchievementsSummaryLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    if (!steamAppId) {
      setAchievementsSummary(undefined);
      setAchievementsSummaryLoading(false);
      return;
    }

    const cacheKey = routeCacheKey("game-profile-achievements-summary", { steamAppId });
    const cached = gameProfileAchievementsSummaryCache.get(cacheKey);
    if (cached !== undefined) {
      setAchievementsSummary(cached);
      setAchievementsSummaryLoading(false);
      return;
    }
    ...
    // 延后启动 Steam 成就 helper，避免用户快速返回游戏库时占用首帧。
    const timer = window.setTimeout(() => {
      if (!active) return;
      setAchievementsSummaryLoading(true);
      invoke<GameAchievementsSummary>("get_game_achievements_summary", { appId: steamAppId })
        .then((summary) => {
          if (!active) return;
          gameProfileAchievementsSummaryCache.set(cacheKey, summary);
          setAchievementsSummary(summary);
        })
        .catch(() => {
          if (!active) return;
          gameProfileAchievementsSummaryCache.set(cacheKey, null);
          setAchievementsSummary(null);
        })
        .finally(() => {
          if (active) setAchievementsSummaryLoading(false);
        });
    }, 1200);
            ...
            <div className="grid grid-cols-4 gap-4 mb-4">
              ...
              <div>
                ...
                {game.steam_app_id == null ? (
                  ...
                  >
                    {achievementsSummaryLoading && achievementsSummary === undefined ? (
                      <span className="inline-flex h-5 items-center">
                        <Icon name="reset" size={14} className="animate-spin text-muted-foreground" />
                      </span>
                    ) : achievementsSummary == null ? (
                      <span className="inline-flex h-5 items-center">
                        <Icon name="reset" size={14} className="animate-spin text-muted-foreground" />
                      </span>
                    ) : achievementsSummary.hasAchievements === false ? (
                      <div className="text-sm">{t("gameDetail.noAchievements", { defaultValue: "无 Steam 成就" })}</div>
                    ) : (
                      <>
                        ...
                        <div className="h-1.5 w-full max-w-[120px] overflow-hidden rounded-full bg-muted">
                          <div
                            ...
                            style={{ width: `${Math.min(Number(achievementsSummary.percentage) || 0, 100)}%` }}
                          />
                        </div>
                      </>
                    )}
