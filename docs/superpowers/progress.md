...
- **三处 UI 调整** —— (1) **播放器截图按钮补 hover 效果**：ScreenshotButton 之前用旧类 `bili-player-icon-button`（控制栏重构后已无样式，无 hover），改为 `bili-ctrl-btn bili-ctrl-btn-label`，与其它控制栏按钮一致（圆形 hover 圆底）。(2) **删除首页热门精选横列**：HomeFeed 移除 `<HotRow/>`，删除 `HotRow.tsx` 与 `.bili-hot-row`/`.bili-hot-track` 样式（首页回到三 Tab + 主网格）。验证：bundle 无 `HotRow`/`bili-hot-row`。(3) **点赞/点踩选项缩小 30%**：`.bili-thumb` min-height 32→22px、padding 10→7px、gap 6→4px、图标 20→14px、字号 11px。验证：`npm run build`/`pack` passed 并写回两目录（MD5 一致），仅 `from "sdk"` 导入。
