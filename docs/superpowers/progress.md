...
- Topic: Steam 商店浏览（商城页：首页轨道 + 分类/排序/打折/区域浏览网格 + 卡片关注/详情）
- Stage 1 Requirement Exploration: Completed at 2026-08-13（经 grill-me 逐项确认 Q1–Q14：完整商店页 / genre 大类免 key / 独立路由 / 卡片直接关注 / 内存缓存 10min / 8 区切换器 / 本地币种 / l 固定 schinese / 双视图）
- Design doc: docs/superpowers/specs/2026-08-13-steam-store-browse-design.md（待用户批准）
- Stage 2 Implementation Planning: Completed at 2026-08-13
- Plan doc: docs/superpowers/plans/2026-08-13-steam-store-browse-plan.md（已批准）
- Stage 3 Plan Execution: **P1 completed at 2026-08-13**（见下）
- **P2 Command 层（T3）** —— review 修正 5 点：(1) SDK BrowseItem 系类型无 Serialize → 命令层 DTO 映射（对齐现有 StoreDetailDto 惯例）；(2) **发现设计矛盾**：设计 §3.3 要求首页随区域切换，但批量 `get_app_details` 固定 cc=cn → SDK 新增 `get_app_details_in_region`（批量 + cc 参数化，fetch_app_detail 重构为区域参数，get_app_details 包装传 cn）；(3) `AppDetail` 缺 platforms → AppDetailData/AppDetail 补 platforms 字段（rails 补齐后平台图标可用）；(4) `FeaturedItem.r#type` 加 `#[serde(default)]`（缺字段防整轨解析失败）；(5) DTO 命名按现有 PriceDto 惯例 finalPrice/initialPrice（非设计草案的 finalPriceCents）。新建 `src-tauri/src/commands/steam_store.rs`：`BrowseItemDto`/`BrowseResultDto`/`FeaturedRailDto`/`StoreHomeDto` + `browse_steam_games(term/category/sort/specials/cc/start/count)`（sort 白名单解析回退 relevance、count clamp 1-50、默认 15）+ `get_store_home(cc)`（featured + featuredcategories rails 经 `get_app_details_in_region` 补齐，空轨跳过）；内存缓存 `OnceLock<Mutex<HashMap>>` TTL 10min（复用 news_cache 模式，类型化非 Value），键 browse|…|count 与 home|cc，错误不缓存；mod.rs 暴露 + lib.rs 注册两命令。验证：`cargo fmt`；`cargo check` 0 errors（无新增 warning）；`cargo test -p steam-sdk store` 17 passed。
- Stage 3 Plan Execution: 下一步 P3（前端数据层）待用户批准
- **P3 前端数据层（T4/T5/T6）** —— review 修正：前端 DTO 对齐 P2 实际输出（finalPrice/initialPrice/currency: string|null，设计 §4.3 草案字段名已由 P2 修正）；`formatPrice` 放 `src/lib/steamCommunity.ts`（现有格式化模块，复用 `CURRENCY_SYMBOLS`，formatPriceCents 保持不动），JPY/KRW 取整无小数（现有注释已确认 Steam 数值按分上报、仅展示去小数）、未知币种回退 `CODE x.xx`；`steamStore.ts` 纯函数封装（非 hook，Store 页面自管状态）。实现：T4 `BrowsePlatformsDto`/`BrowseItemDto`/`BrowseResultDto`/`FeaturedRailDto`/`StoreHomeDto`；T5 新建 `src/lib/steamStore.ts`（`browseSteamGames(params)` 显式构建 Record 参数 + `getStoreHome(cc?)`；首轮 tsc 报 `BrowseSteamGamesParams` 无 index signature 不符合 InvokeArgs → 修复为按 key 显式构造 Record<string, unknown>，顺带避免传 undefined）；T6 `formatPrice(cents, currency)`。验证：`npm run build` passed（tsc 0 errors + vite + plugin-sdk）。
- Stage 3 Plan Execution: 下一步 P4（页面骨架/路由/导航/i18n）待用户批准
- **P4 页面骨架（T7/T8/T9）** —— review 修正：`assets/regular/storefront.svg`（Phosphor 集）已存在，直接注册为 `store`（无需回退方案）；SidebarMenu 的 useMemo deps 补 `storeLabel`（否则 memo 不更新）；新侧边栏项不加 `end`（/steam/store 叶子路由）。实现：T7 新建 `src/pages/steam/Store.tsx`（GlassCard 标题 + 空态占位，P5–P7 填充）+ `App.tsx` 注册 `/steam/store`；T8 `SidebarMenu.tsx` 加 `storeLabel` prop + steam 组 `{key:"store", to:"/steam/store", icon:"store"}` + deps，`Layout.tsx` 侧边栏与水平导航两处传 `t("steam.storeLabel")`；T9 `icons.ts` import storefront.svg 注册 `store` + i18n zh/en 一次性补全 P4–P7 文案键（storeLabel/storePageTitle/5 轨道名/工具栏 7 键/排序 5 键/分类 12 genre/区域 8 区/加载更多/已加载全部/空态/错误/重试/关注/已关注/暂无价格）。验证：`npm run build` passed（tsc 0 errors）。
- Stage 3 Plan Execution: 下一步 P5（首页视图 4 轨道）待用户批准
- **P5 首页视图（T10）** —— review 修正：GameBannerCard 语义（游玩/备份卡）但 props 完全够用（appId/name/badges/rightContent + header.jpg CDN 回退），精选轨道直接复用它；单轨失败已由后端 P2 处理（空轨跳过），前端只做整体 error/retry。实现：`steamStore.ts` 加 `capsuleImageUrl`/`headerImageUrl`（CDN 直连，P7 网格复用）；`Store.tsx` 重写——`getStoreHome()` 加载 + loading 骨架（大卡 + 轨道占位）/ error（storeError + storeRetry 按钮）/ 数据三分支；精选轨道 GameBannerCard 横滚大卡行（折扣 badge + 价格 rightContent）；4 轨 RailRow 横滚小卡行（StoreItemCard：capsule 图 lazy + onError 回退 header + 名称 + PriceInfo 折扣徽标/现价/原价删除线/暂无价格）；rail 标题 id→i18n 键映射（specials/coming_soon/top_sellers/new_releases），未知回退后端 name。验证：`npm run build` passed（tsc 0 errors）。**手测待补**：tauri dev 首页 4 轨道加载与横滚（本环境 store 域被 Akamai 风控，无法自动验证）。
- Stage 3 Plan Execution: 下一步 P6（浏览网格工具栏 + 分页）待用户批准
- **P6 浏览网格（T11/T12）** —— review 决策：视图切换规则 = term/category/sort/specials 任一非默认 → 网格，否则首页；**区域切换只在网格模式**（首页固定 cn——设计 §5.2 结构图把工具栏放网格视图内，且 grill Q7 共识的切换器服务浏览列表；设计 §5.2"切换即重置首页或网格"落实为重置网格）；补 i18n `storeCategoryAll`（P4 遗漏的"全部"分类项）；StoreItemCard 重构为自适应宽度（横滚行由外层 `flex-none w-64` 定宽，网格用 `grid-cols-[repeat(auto-fill,minmax(210px,1fr))]`）。实现：Store.tsx 加工具栏（搜索 TextField 防抖 350ms 参照 GameSearchBox → debouncedTerm；分类 Select 12 genre；排序 Select 5 白名单；只看打折 Toggle 包 label；区域 Select 8 区）+ 网格（browseSteamGames 首屏 start=0，条件变化经 `requestIdRef` 序号防竞态重置；加载更多 start=items.length 累积，按钮 loading 态、失败保留可重试、`items.length >= total` 显示"已加载全部"；首屏骨架 6 块；空态 total==0；错误态 + 重试；首页与网格错误独立）。验证：`npm run build` passed（tsc 0 errors）。**手测待补**：tauri dev 搜索/分类/排序/打折/区域切换、翻页、空态（本环境 store 域 Akamai 风控）。
- Stage 3 Plan Execution: 下一步 P7（卡片交互：关注按钮 + StoreDetailDialog）待用户批准
- **P7 卡片交互（T13/T14）** —— review 修正：关注按钮用**图上角文字 chip**（BookmarkToggle 是备份监控书签语义，混用会混乱；i18n storeFollow/storeFollowed 已建）；showToast 实际在 `src/lib/toast.ts` 且签名 (type, message)（首轮 tsc 报 TS2554 → 修正导入与调用）。实现：icons.ts 注册 phosphor `windows`/`apple`/`linux` 平台图标（assets 已有 windows-logo/apple-logo/linux-logo.svg）；StoreItemCard 加 `onClick`（hover ring + cursor）/`followed`/`onToggleWatch`，图上角关注 chip（已关注 primary 高亮、未关注半透明黑底，stopPropagation）、Metacritic 绿徽标（名称右侧）、平台图标行（有则显示）；Store 根加 `useSteamHubCache().watchItems` + `useSteamWatchlist` + `watchedIds` Set + `detailAppId` + `toggleWatch`（乐观 patch steamHubCache → await add/remove（内部 refresh 收敛真实值）→ catch 回滚 + `showToast("error")`）+ `StoreDetailDialog`（卡片本体点击打开）；网格与 Rails 全部接线。补 i18n `storeFollowFailed`（zh/en）。验证：`npm run build` passed（tsc 0 errors）。**手测待补**：tauri dev 关注/取消 → Steam Hub 新闻/愿望单即时同步、点卡片开详情（本环境 store 域 Akamai 风控）。
- Stage 3 Plan Execution: P1–P7 全部完成，下一步 P8（手测冒烟）待用户批准
- **P7 修复 5：显式浏览入口（2026-08-13，用户批评双视图无入口设计缺陷后确认方案 A）** —— 双视图隐式切换（条件生效才显示网格/工具栏）无可见入口是设计缺陷（P6 review 已知矛盾但按文档执行）。按用户选择方案 A 修复：新增显式 `browseMode` state，`browsing = filtersActive || browseMode`；首页精选轨道标题行右侧加"浏览全部游戏"按钮（点击进无筛选网格）；工具栏右侧在 `browsing && !filtersActive` 时显示"返回首页"按钮（清空条件后退出浏览模式回首页）。i18n 加 storeBrowseAll/storeBackHome（zh/en）。验证：`npm run build` passed（tsc 0 errors）。
- **P7 修复 6：Select 下拉选项点击无反应（2026-08-13，用户澄清"点击下拉选项没有任何反应"）** —— 根因：**点击外部关闭竞态**——选项按钮在 portal（body）里，不在 Select 容器 ref 内，`document mousedown` 关闭 handler 把"点选项"误判为"点外部"→ mousedown 时 setOpen(false) → React 渲染后面板 `pointer-events-none` → mouseup 时按钮不可点击 → **浏览器不派发 click → onChange 永不触发**。此 bug 影响所有使用 Select 的页面（设置页同样受影响，只是未被报告）。修复：选项按钮 `onMouseDown={(e) => e.stopPropagation()}` 阻止冒泡到 document（`src/components/ui/Select.tsx`），全局受益。验证：`npm run build` passed（tsc 0 errors）。
- **P7 修复 7：筛选结果不变（2026-08-13，用户实测"所有选项筛选后显示的都是同样的游戏"）** —— 实证根因：**`/api/storesearch` 静默忽略 `category1`/`sort_by`/`specials` 参数**（term 有效、筛选无效；所有筛选请求返回同样的"全部"结果）。修复：浏览网格数据源改为**搜索页 JSON 接口 `GET /search/results/?query=&category1=&sort_by=&specials=&cc=&l=&start=&count=&infinite=1`**（官方搜索页接口，全部筛选参数真实生效；第三方 AutomatiK/NeedFree 同款用法）：SDK `search_results_page` 返回 `(total_count, app_ids)`（results_html 提取 `data-ds-appid`）；command 层 `browse_steam_games` 经 `get_app_details_in_region` 批量补齐详情（每页 15 条并行 8 + 10 分钟缓存兜底，appdetails 结构化数据比 HTML 价格可靠）；删除 storesearch 版 `browse_games`/`BrowseResult`/`BrowseItem::from_search_item`（`search_games` 保留——storesearch 对 term 有效，搜索建议继续用）。修复过程两处小修：URL 测试断言（空 term 时 `query=` 空值合法=浏览全部）、`cc` 被 move 进 BrowseParams 后借用（clone）。新增测试：URL 构造（search/results 全参数）、`extract_app_ids`（fixture 含 data-ds-appid）、空结果 case。验证：`cargo fmt`；`cargo test -p steam-sdk store` 19 passed；`cargo check` 0 errors；`npm run build` passed。
- **P7 修复 8：分类参数错误（2026-08-13，用户实测"只有冒险分类不一样，其它分类都还是一样的游戏"）** —— 定位：`category1` 不是 genre 筛选！**它是"内容类型"筛选**（权威证据 scrape_up/steamScraper.py：`category1=998`=Games、`category1=21`=**DLC**、`category1=10`=Demos、`category1=994`=Software、`category1=997`=Mods）。用户看到"冒险分类不一样"实为 category1=21 筛选出了 DLC；其余 19/23/24... 无效值被忽略返回全部。**genre 的正确参数是 `tags`**（RSSHub 同款 `tags=492` 用法）。修复：SDK `build_browse_url` 分类参数 `category1={}` → `tags={}`，并**固定 `category1=998`（Games）**过滤掉 DLC/演示/软件（浏览全部也受益）；前端 GENRES 移除 998 项（998 是 Games 内容类型，**不是** Free to Play genre——此前表项错误）。测试断言同步（tags=19 + 固定 category1=998）。验证：`cargo test -p steam-sdk store` 19 passed；`cargo check` 0 errors；`npm run build` passed。
- **P7 修复 9：genre tag id 错误（2026-08-13，用户实测"只有动作、冒险分类有游戏，其他分类空"）** —— 根因：GENRES 表的 id（23/24/25/28/29/30/31/32/38）是**记忆混淆的 appdetails genre id**，不是搜索页 `tags` 参数的 tag id——只有 Action=19、Adventure=21 碰巧正确。**权威映射**（steam-cli `assets/tags.popular.en.json`，446 个官方热门 tag）：Action=19、Adventure=21、Indie=**492**、RPG=**122**、Strategy=**9**、Casual=**597**、Simulation=**599**、Sports=**701**、Racing=**699**、Massively Multiplayer=**128**、Horror=**1667**、Free to Play=**113**（RSSHub example 的 tags=492 即 Indie，佐证）。修复：前端 GENRES 替换为真实 tagid，并补回 Free to Play=113（此前 998 误标已移除）。后端 tags={id} 逻辑不变；测试 URL 断言（tags=19）不变。验证：`npm run build` passed（tsc 0 errors）。**手测待补**：tauri dev 12 个分类各自筛出对应游戏（用户机器验证中）。
- **P1 SDK（T1/T2）** —— store.rs 扩展：`StoreSearchPrice` 补 initial/discount_percent/currency、`StoreSearchItem` 补 release_date/platforms/metacritic_score、`StoreSearchResponse.total`；新增 `Platforms`、`BrowseSort`（5 枚举 + as_param）、`BrowseParams`（term/category/sort/specials/cc/start/count，Default cc=cn count=15）、`BrowseItem`、`BrowseResult{total,items}`、纯函数 `build_browse_url`、`browse_games`（/api/storesearch 参数化）、`featured`（/api/featured 扁平价字段）、`FeaturedRail` + `featured_categories`（/api/featuredcategories，`specails` 拼写 alias，取 4 轨跳过 genres，空轨丢弃）。新增 5 单测（sort 映射/URL 构造/browse 解析含缺字段容错/featured 解析/featuredcategories 解析）。**T2 环境限制**：本执行环境出口 IP 被 store.steampowered.com（Akamai CDN）整体风控（appdetails/featured/homepage 全 403，curl 与 ureq 均被拒；api.steampowered.com/steamcommunity.com/CDN 图片域可达）→ 真实参数验证无法在本环境执行，按设计 §8 回退：离线单测锁定解析与 URL 构造，真实请求验证移入 P8 手测（用户机器现有 search_games 已验证 storesearch 可达）。探针 examples/store_probe.rs 已删除。验证：`cargo test -p steam-sdk store` 17 passed；`cargo test -p steam-sdk` 103 passed；`cargo fmt`；`cargo check` 0 errors。
- Stage 3 Plan Execution: 下一步 P2（steam_store.rs 命令层）待用户批准

## Previous Workflow Request（归档）
