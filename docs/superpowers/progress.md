...
- Stage 3 Plan Execution: P0/P1/P2 completed at 2026-08-11（见下）；**P3 completed at 2026-08-12**（自动验证通过，Tauri 真实账号手测待补）
...
- P3 verification: `cargo fmt`；`cargo check` passed（0 errors）；`cargo test bilibili`（41 passed，新增 4 个 protobuf 解析/转换单测）；`cargo test plugins`（18 passed）；`npm run build` passed；插件 `npm run build && npm run pack` passed 并写回两目录（bundle MD5 一致 4724cb38）；`npm run test:danmaku` passed（滚动+固定弹幕全用例）；bundle 仅 `from "sdk"` 导入（44 处）、0 处 `@tauri-apps`、含 4 个新 SDK 调用与 `bili-danmaku-self/menu/fixed`、`DanmakuSegmentLoader`；宿主 plugin-sdk.js 含 4 个新命令名；`npx tsc --noEmit` 对 P3 改动文件无新增错误（既有 JSX.IntrinsicElements/P2 遗留除外）。
- P3 pending hand-test（`npm run tauri dev` 人工执行）：长视频播放中分段弹幕随播放窗口逐段加载（可观察 Network 分段请求）、seek 跳段后弹幕同步、顶部/底部固定弹幕渲染位置、发送弹幕 5 分钟内描边 + 点赞/举报/撤回、撤回后弹幕消失、未登录点自己弹幕提示、以及既有弹幕显示/发送/设置与播放不回退。
