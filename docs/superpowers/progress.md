...
- **P7 修复 4：封面不显示 + 闪烁（2026-08-13，用户实测反馈）** —— 根因：走 Steam **新资产系统**的游戏没有 `apps/{appid}/capsule_616x353.jpg` 路径（真实 URL 带 hash 目录 + `_schinese` 后缀，见用户反馈的 `large_capsule_image` 样本），前端拼的 CDN URL 404 → 破图；且图加载完成时从灰块突变（无过渡）+ onError 切换 → 视觉闪烁。修复：`StoreItemCard` 图源三级 fallback——拼 capsule(清晰) → 接口 `tinyImage`(真实有效) → 拼 header，全部失败显示占位图标（stage state）；img 加 `transition-opacity` 300ms + onLoad 淡入（突变变平滑）；`GameBannerCard` 加 `imageUrl?: string` prop（header → 接口图 → capsule → 首字母占位，同样淡入），Store featured 大卡传 `item.tinyImage`。验证：`npm run build` passed（tsc 0 errors）。**手测待补**：tauri dev 商城页封面完整显示无闪烁（用户机器验证中）。
